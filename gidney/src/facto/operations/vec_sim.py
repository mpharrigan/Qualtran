...
class VecSim:
    ...
    def peek_obs(self, obs: Dict[Any, Literal["X", "Y", "Z"]], *, sign: int = +1) -> float:
        s = self.copy()
    ...
    def do_ch(self, c: Any, q: Any) -> None:
        f = self.state_slicer({c: True, q: False})
        t = self.state_slicer({c: True, q: True})
        self._buffer[f] = self.state[f]
        self.state[f] += self.state[t]
        self.state[t] *= -1
        self.state[t] += self._buffer[f]
        self.state[self.state_slicer({c: True})] *= np.sqrt(0.5)
    ...
    def do_mxx(
        self, a: Any, b: Any, *, key: Optional[Any] = None, prefer_result: bool | None = None
    ) -> bool:
    ...
    def do_pauli_dot(
        self, *, paulis: Sequence[str], controls: Sequence[bool], targets: Sequence[Any]
    ):
