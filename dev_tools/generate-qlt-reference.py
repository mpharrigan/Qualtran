...
"""Generate `.qlt` reference files for the QLT IR example bloqs.

For each example in `qualtran.qlt_ir.QLT_IR_EXAMPLES` this:
 ...
 3. Writes (or, with `--check`, verifies) the committed reference file under
    `qualtran/qlt_ir/examples/`.

Usage:

```
python dev_tools/generate-qlt-reference.py           # (re)write
python dev_tools/generate-qlt-reference.py --check   # verify only
```
"""
...
def render_reference(example: QltExample) -> str:
...
def generate(check: bool = False, include_slow: bool = True) -> List[str]:
    ...
    """
    stale: List[str] = []
    for example in get_qlt_ir_examples(include_slow=include_slow):
...
def main():
    ...
    if args.check and stale:
        print('The following QLT IR reference files are stale:', file=sys.stderr)
        for s in stale:
            print(f'  - {s}', file=sys.stderr)
        print('Regenerate them with: python dev_tools/generate-qlt-reference.py', file=sys.stderr)
        sys.exit(1)
