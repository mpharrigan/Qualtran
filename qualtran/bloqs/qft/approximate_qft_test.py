...
def test_approximate_qft_exact(n: int, without_reverse: bool):
    ...
    np.testing.assert_allclose(qft_bloq.tensor_contract(), cirq.unitary(qft_cirq))
    np.testing.assert_allclose(qft_bloq.adjoint().tensor_contract(), cirq.unitary(qft_cirq**-1))
