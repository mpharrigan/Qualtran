...
def test_pretty_print_roundtrip(original_code):
    ...
    pretty_printed_code = QltASTPrinter().visit(original_ast)
...
def test_lvalue_annotation_rendered():
    ...
    assert QltASTPrinter().visit(node) == 'q @ circle'


def test_lvalue_without_annotation():
    from qualtran.qlt_ir.nodes import LValueNode

    assert QltASTPrinter().visit(LValueNode(name='q')) == 'q'
...
def test_qarg_annotation_rendered():
    ...
    assert QltASTPrinter().visit(node) == 'x=q @ circle'
...
def test_qcall_annotation_rendered():
    ...
    _rets, mid, _qargs = QltASTPrinter().visit(node)
...
def test_empty_tuple_node():
    ...
    assert QltASTPrinter().visit(TupleNode(items=[])) == '()'
