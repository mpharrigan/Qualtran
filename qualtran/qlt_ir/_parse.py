...
class QltParser:
    ...
    def __init__(self, tokens: List[Token], nodes: QltNodes = qualtran_qlt_nodes):
        self.tokens = tokens
        self.pos = 0
        self.nodes = nodes
    ...
    def parse_module(self) -> QltModule:
        """Parse a QLT IR module consisting of multiple quantum definitions.

        A module is a sequence of `qdef` or `extern qdef` blocks representing
        a complete QLT IR file or program.

        Returns:
            A QltModule containing the parsed quantum definitions.
        """
        if self.check('EOF'):
            return self.nodes.QltModule(qdefs=[])

        qdefs = self.parse_qdefs()

        self.consume('EOF', "Expected EOF")
        return self.nodes.QltModule(qdefs=qdefs)
...
def parse_objectstring(objectstring: str, *, nodes: QltNodes = qualtran_qlt_nodes) -> CObjectNode:
    ...
    parser = QltParser(tokens, nodes=nodes)
    cval_node: CObjectNode = parser.parse_cobject_only()
    return cval_node


def parse_module(qlt_code: str, *, nodes: QltNodes = qualtran_qlt_nodes) -> QltModule:
    """Parse an entire QLT IR code string into a QltModule.

    Args:
        qlt_code: The QLT IR source code as a string.
        nodes: The module providing the AST node constructors.

    Returns:
        A QltModule containing the root AST of the parsed code.
    """
    tokens = tokenize(qlt_code)
    parser = QltParser(tokens, nodes=nodes)
    return parser.parse_module()


def _qlt_to_json_dict(self):
    d = {'_qlt_node': self.__class__.__name__}
...
def qlt_ast_node_to_json(o: object):
    from .nodes import QltASTNode

    if isinstance(o, QltASTNode):
        return _qlt_to_json_dict(o)
    return o


def dump_ast(node: QltASTNode, f):
    json.dump(node, f, indent=2, default=qlt_ast_node_to_json)
