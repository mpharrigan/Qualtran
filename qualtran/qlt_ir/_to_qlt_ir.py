...
def regs_to_sig_entry(
    reg_name: str, regs: Sequence['qlt.Register'], *, nodes: QltNodes = qualtran_qlt_nodes
) -> QSignatureEntry:
...
def signature_to_qlt_ir_entries(
    signature: 'qlt.Signature', *, nodes: QltNodes = qualtran_qlt_nodes
...
class QltModuleBuilder:
    """Format and export a QLT IR 'module': a collection of 'qdef' bloq definitions."""
    ...
    def __attrs_post_init__(self):
        ...
        elif self.qglobals.nodes is qualtran_qlt_nodes and self.nodes is not qualtran_qlt_nodes:
    ...
    def finalize(self) -> QltModule:
        return self.nodes.QltModule(qdefs=tuple(qdef_with_ctx.qdef for qdef_with_ctx in self.qdefs))
    ...
    def pretty_print_qdef(self, bloq_or_bloq_key: Union[qlt.Bloq, BloqKey], f=None) -> None:
        ...
        qdef_with_ctx = qdefs[bloq_key]
        qdef_node = qdef_with_ctx.qdef  # remove context
        txt = QltASTPrinter().visit(qdef_node)
        if f is not None:
            f.write(txt)
        else:
            print(txt)
    ...
    def __str__(self):
        ...
        return f'QltModuleBuilder({bloq_keys})'
...
def _guess_good_alias_prefix(bloq: qlt.Bloq, *, nodes: QltNodes = qualtran_qlt_nodes):
