...
class QltVisitorBase(metaclass=abc.ABCMeta):
    @functools.singledispatchmethod
    def visit(self, node: QltASTNode) -> Dict[str, Any]:
        record = attrs.asdict(cast(Any, node), recurse=False)
        record = {k: self.visit(v) if isinstance(v, QltASTNode) else v for k, v in record.items()}
        return record

    @visit.register
    def _(self, node: QltModule):
