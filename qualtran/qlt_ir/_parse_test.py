...
from qualtran.qlt_ir._parse import (
    dump_ast,
    parse_module,
    parse_objectstring,
    qlt_ast_node_to_json,
    QltParser,
    Token,
    tokenize,
)
...
def test_parse_newlines_comments():
    ...
    node = QltParser(tokens).parse_cobject_node()
    assert isinstance(node, CObjectNode)
...
def test_ast_node_to_json():
    ...
    dump_ast(node, f)
    result = f.getvalue()
    assert '"_qlt_node": "CArgNode"' in result
    assert '"key": "x"' in result

    # Test fallback
    assert qlt_ast_node_to_json("string") == "string"


def test_parse_int_literal_invalid():
    parser = QltParser([Token('NUMBER', '1.5', 1, 0)])
    with pytest.raises(ValueError, match="Expected an integer literal"):
        parser.parse_int_literal()


def test_parse_cvalue_cobject():
    parser = QltParser([Token('NAME', 'MyBloq', 1, 0), Token('EOF', '', 1, 0)])
    node = parser.parse_cvalue()
    assert isinstance(node, CObjectNode)
    assert node.name == 'MyBloq'
...
def test_parse_nested():
    from qualtran.qlt_ir._eval import eval_cvalue_node
...
def test_parse_module_empty():
    parser = QltParser([Token('EOF', '', 1, 0)])
...
def test_parse_lvalues_pipe():
    parser = QltParser(tokenize("| = MyBloq()"))
    lvals = parser.parse_lvalues()
    assert lvals == []


def test_parse_qarg_nested():
    parser = QltParser(tokenize("target=[x[0], [y[1], z[2]]] ]"))
...
def test_parse_qarg_with_annotation():
    parser = QltParser(tokenize("ctrl=qvar @ 1"))
    qarg = parser.parse_qarg()
    ...
    parser = QltParser(tokenize("reg[0]=qvar @ 2"))
    qarg = parser.parse_qarg()
