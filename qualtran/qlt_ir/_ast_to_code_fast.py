...
"""Direct recursive AST-to-code formatting for QLT IR.

This module converts QLT IR AST nodes directly into their textual code
...
def format_module(node: QltModule) -> str:
    """Format an entire QltModule node into QLT IR source code."""
    s = '# QLT IR\n# 1.0.0\n\n'
    s += '\n\n'.join(format_qdef(qdef) for qdef in node.qdefs)
    return s


def format_node(node: QltASTNode) -> Any:
    """Format any QLT IR AST node."""
    t = type(node)
    if t is QltModule:
        return format_module(cast(QltModule, node))
...
class FastQltASTPrinter:
    """Walk a QLT IR AST using direct recursive functions to generate IR text."""

    def visit(self, node: QltASTNode) -> Any:
