...
from .nodes import CArgNode, CObjectNode, CValueNode, QltNodes, TupleNode
...
def object_to_object_node(
    ...
    *,
    ...
    and recursively converts its values to QLT IR CValueNode AST nodes.
...
def unserializable_object(name: str, *, nodes: QltNodes = qualtran_qlt_nodes) -> CObjectNode:
...
def tuple_to_tuplenode(t: tuple, *, nodes: QltNodes = qualtran_qlt_nodes) -> TupleNode:
...
def ndarray_to_ndarr_objectnode(
    a: np.ndarray, max_size: int = 100, *, nodes: QltNodes = qualtran_qlt_nodes
) -> CObjectNode:
...
def to_cobject_node(x: Any, *, nodes: QltNodes = qualtran_qlt_nodes) -> CValueNode:
...
def dump_objectstring(x: Any, *, nodes: QltNodes = qualtran_qlt_nodes) -> str:
