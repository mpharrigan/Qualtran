...
"""Tests for the QLT IR round-trip utilities, examples, and reference files.
...
This module has three parts:
 ...
 2. Parametrized round-trip tests over the curated `QLT_IR_EXAMPLES`.
...
import qualtran.qlt_ir._roundtrip as _rt
...
from qualtran.qlt_ir._eval import _PlaceholderBloq
from qualtran.qlt_ir._examples import QLT_IR_EXAMPLES, reference_path
from qualtran.qlt_ir._parse_eval import load_module
from qualtran.qlt_ir._roundtrip import (
...
_EXAMPLE_PARAMS = [
    pytest.param(ex, id=ex.name, marks=[pytest.mark.slow] if ex.slow else [])
    for ex in QLT_IR_EXAMPLES
]
...
def test_compile_bloq_to_qlt_ir_fields():
    artifacts = compile_bloq_to_qlt_ir(CNOT())
    assert artifacts.root_bloq_key == 'CNOT'
    assert 'CNOT' in artifacts.original
    assert 'CNOT' in artifacts.loaded
    assert artifacts.qlt_code.startswith('# QLT IR')
...
def test_check_bloq_keys_clean():
    assert check_bloq_keys(compile_bloq_to_qlt_ir(CNOT())) == []


def test_check_bloq_keys_missing():
    artifacts = compile_bloq_to_qlt_ir(CSwap(bitsize=2))
...
def test_check_signatures_clean():
    assert check_signatures(compile_bloq_to_qlt_ir(CNOT())) == []
...
def test_check_no_placeholders_clean():
    assert check_no_placeholders(compile_bloq_to_qlt_ir(CNOT())) == []
...
def test_check_bloq_object_identity_clean():
    assert check_bloq_object_identity(compile_bloq_to_qlt_ir(CSwap(bitsize=2))) == []
...
def test_check_bloq_object_identity_skips_qcast():
    ...
    artifacts = compile_bloq_to_qlt_ir(BitwiseNot(qdt.QUInt(4)))
...
def test_check_soquet_graph_isomorphism_clean():
    assert check_soquet_graph_isomorphism(compile_bloq_to_qlt_ir(CSwap(bitsize=2))) == []
...
def test_save_bloq_qlt_writes_and_reloads(tmp_path):
    ...
    assert code == compile_bloq_to_qlt_ir(CSwap(bitsize=2)).qlt_code
...
def test_reference_file_up_to_date(example):
    ...
    assert path.is_file(), (
        ...
        f'`python dev_tools/generate-qlt-reference.py`.'
    )

    bloq = example.make()
    validate_bloq(bloq)
    artifacts = compile_bloq_to_qlt_ir(bloq)
    assert check_artifacts(artifacts) == []
    assert path.read_text() == artifacts.qlt_code, (
