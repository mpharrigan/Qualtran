...
"""Utilities for round-trip testing the QLT intermediate representation.

The QLT IR pipeline has four stages:

```
Bloq  --compile-->  QltModule AST  --print-->  .qlt text
.qlt text  --parse-->  QltModule AST  --eval-->  {bloq_key: Bloq}
```
...
from . import nodes as qualtran_qlt_nodes
from ._ast_to_code import QltASTPrinter
from ._parse_eval import load_module
from ._to_qlt_ir import QDefWithContext, QltModuleBuilder
from .nodes import QltNodes


@attrs.frozen
class RoundtripArtifacts:
    """The intermediate products of round-tripping a bloq through QLT IR.
    ...
    Attributes:
        ...
        qlt_code: The textual `.qlt` representation.
        loaded: The bloqs recovered by parsing and evaluating `qlt_code`, keyed by
            bloq key.
    """
...
def compile_bloq_to_qlt_ir(
    bloq: 'qlt.Bloq', *, extern_only_from: bool = False, nodes: 'QltNodes' = qualtran_qlt_nodes
) -> RoundtripArtifacts:
    ...
    *not* run any property checks. Use `check_bloq_roundtrip` to additionally
    ...
    Args:
        ...
        extern_only_from: Passed through to `QltModuleBuilder.add_bloqs`.
        nodes: AST node factory module.

    Returns:
        A `RoundtripArtifacts` capturing every intermediate product.
    """
    # Use QltModuleBuilder directly so we retain a record (`original`) of the
    # true subbloqs we serialized, alongside their Python objects.
    qlt_mb = QltModuleBuilder(nodes=nodes)
    root_bloq_key = qlt_mb.add_bloqs(root=bloq, extern_only_from=extern_only_from)
    original = {qdef.bloq_key: qdef for qdef in qlt_mb.qdefs}

    qlt_code = QltASTPrinter().visit(qlt_mb.finalize())

    loaded = load_module(qlt_code)

    return RoundtripArtifacts(
        root_bloq_key=root_bloq_key, original=original, qlt_code=qlt_code, loaded=loaded
    )
...
def check_bloq_roundtrip(bloq: 'qlt.Bloq', *, extern_only_from: bool = False) -> List[str]:
    """Round-trip a bloq through QLT IR and return a list of problems.
    ...
    Args:
        ...
        extern_only_from: Passed through to `compile_bloq_to_qlt_ir`.

    Returns:
        A (possibly empty) list of human-readable problem descriptions.
    """
    artifacts = compile_bloq_to_qlt_ir(bloq, extern_only_from=extern_only_from)
...
def assert_bloq_roundtrips(bloq: 'qlt.Bloq', *, extern_only_from: bool = False) -> None:
    """Assert that a bloq round-trips through QLT IR with no problems.
...
def save_bloq_qlt(
    ...
    *,
    ...
    out_path.write_text(artifacts.qlt_code)
    return artifacts.qlt_code
