...
def test_inverse_flows():
    chunk = gen.Chunk(
        ...
        flows=[gen.Flow(center=0, start=gen.PauliMap({}), mids=[0], end=gen.PauliMap({1: "Z"}))],
    )
...
def test_reflow():
    ...
    chunk1 = gen.Chunk(
        ...
        flows=[gen.Flow(end=xx, mids=[0], center=0), gen.Flow(end=zz, mids=[1], center=0)],
    )
    chunk2 = gen.Chunk(
        q2i={0: 0, 1: 1},
        circuit=stim.Circuit(
        ...
        flows=[gen.Flow(start=yy, mids=[0], center=0)],
        discarded_inputs=[xx],
    )
...
def test_from_circuit_with_mpp_boundaries_simple():
    ...
    assert chunk == gen.Chunk(
        ...
        flows=[
            gen.Flow(
                start=gen.PauliMap.from_xs([1 + 2j]),
                end=gen.PauliMap.from_zs([1 + 2j]),
                mids=(),
                obs_key=None,
                center=1 + 2j,
            )
...
def test_anticommuting_obs_flows():
    chunk = gen.Chunk(
        ...
        flows=[
            gen.Flow(start=gen.PauliMap({"X": [0, 1, 1j, 1 + 1j]}), mids=[0]),
            gen.Flow(end=gen.PauliMap({"X": [0, 1, 1j, 1 + 1j]}), mids=[0]),
            gen.Flow(start=gen.PauliMap({"Z": [0, 1]}), mids=[1]),
            gen.Flow(end=gen.PauliMap({"Z": [0, 1]}), mids=[1]),
            gen.Flow(start=gen.PauliMap({"Z": [1j, 1 + 1j]}), mids=[2]),
            gen.Flow(end=gen.PauliMap({"Z": [1j, 1 + 1j]}), mids=[2]),
            gen.Flow(
                start=gen.PauliMap({"X": [0, 1]}), end=gen.PauliMap({"X": [0, 1]}), obs_key="X"
            ),
            gen.Flow(
                start=gen.PauliMap({"Z": [0, 1j]}), end=gen.PauliMap({"Z": [0, 1j]}), obs_key="Z"
            ),
        ],
    )
    chunk.verify()
...
def test_embedded_observables():
    chunk = gen.Chunk(
        ...
        flows=[gen.Flow(start=gen.PauliMap.from_zs([0]).with_name("L2"))],
        o2i={"L2": 2},
    )
    chunk.verify()
