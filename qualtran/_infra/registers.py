...
class Signature:
    ...
    For example, an in-place 32-bit quantum adder may have a signature like:
    ```python
    Signature([
      Register('a', QUInt(32)),
      Register('b', QUInt(32)),
    ...
    def build(cls, **registers: Union[int, sympy.Expr]) -> 'Signature':
        ...
        For rapid prorotyping or simple gates, this syntactic sugar can be used. The following
        call
        ```python
        Signature.build(a=32, b=1)
        ```
        is equivalent to
        ```python
        Signature([
